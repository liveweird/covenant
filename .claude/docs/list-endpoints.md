...
- Filtering: whitelisted equality filters on the field's own name plus `field[gte|gt|lte|lt]` bracket operators only where needed; strict `true`/`false` booleans; enums by string name; repeated key reserved for `IN` — no param has documented `IN` semantics in the scaffold; the contracts list's `type` and `lifecycle` (any-of) will, read via `repeatedValues`; on every other scalar key repetition stays a `400` (Lettuce's `singleValue` rule — never silent first-value-wins; unknown parameter *names* stay ignored by deliberate leniency) [API-LIST-004].
