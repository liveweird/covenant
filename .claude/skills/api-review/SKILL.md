---
name: api-review
description: Validate an OpenAPI spec (default the covenant contract) against api-guidelines/API-GUIDELINES.md — Spectral lint pass plus the LLM review checklist, reported per rule ID.
---
...
file/spec citation. (Say N-A explicitly where a check has no subject yet — e.g. list checks before the contract
list lands — rather than skipping it silently.) The checker sidecar's own tiny contract,
`checker/openapi.yaml`, is reviewed the same way when it changes.
