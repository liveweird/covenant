...
The catalog is hierarchical: **Domain → System → Contract → Version**. Domains and Systems are ADMIN-curated registries (a System belongs to one Domain); a Contract belongs to one System, has a type (`OPENAPI | ASYNCAPI | ODCS`) and an **owner** — a team, or (less often) an individual user. Every authenticated user reads everything; **writing a contract or its versions requires membership of the owning team, being the owning user, or ADMIN** (ownership transfer is ADMIN-only). Teams are flat (membership only — no management chain). Version rules are strict: numbers are unique per contract by SemVer precedence (ignoring build metadata), and older-version backports may be added after higher versions; the document text is editable only in DRAFT/PROPOSED (a DRAFT may be deleted); from ACTIVE on only the lifecycle moves. Validation is two-tier (Toadie's model): **HARD** findings — unparseable YAML/JSON, a document of the wrong type — are a `400` on every store path; **SOFT** findings (schema, semantic, lint) block a strict save but are waivable with `allowInvalid=true` (the editor's Save-anyway modal; import always waives). Findings are produced by the JVM (Jackson parse, swagger-parser for OpenAPI, networknt JSON Schema validation of AsyncAPI/ODCS against vendored official schemas, Apache Avro for payload schemas) merged with the **checker sidecar**'s verdicts (`checker/`: Spectral rulesets + `@asyncapi/parser`, reached only by the server over an internal network — an unreachable checker yields a report-only `CHECKER_UNAVAILABLE` finding, never a failed request). See `.claude/docs/contract-standards.md` for the formats and `.claude/docs/checker.md` for the sidecar.

**Implemented so far:** the full stack + tooling + auth surface — JWT login with a sliding refresh pair and a revocation blocklist, opt-in email MFA, self-service password reset, per-account lockout and per-IP rate limits, ADMIN-managed accounts and per-user feature flags, the synced per-user UI/email language (EN/PL), the React shell (nav model, command palette, theme, changelog), the checker service (Spectral + `@asyncapi/parser` behind `POST /check`), **flat teams** (ADMIN-curated registry with rosters — the ownership unit), the **Domain → System registries**, and the **contract catalog server side** (contracts with team/user ownership and the writer guard, SemVer versions with the lifecycle, the two-tier check pipeline — JVM parse/type gate/schema/semantic validators + the checker sidecar's lint — with the `allowInvalid` waiver, breaking-change detection against the published baseline, the repo reference + sync-from-source (V12: the SPA fetches the reference through the guarded fetch, attributes the difference to a side against the stored baseline, and overwrites an editable version or seeds a new one), the raw-content download, import report-&-skip with a dry run, the SSRF-guarded URL fetch, export, the per-contract event log, **followers + in-app notifications** (every event on a followed contract reaches the follower's bell, never the actor), **facet counts** on the catalog filters, the ADMIN-curated **Environments** registry (HTTP / Kafka / PostgreSQL targets per system, passwords encrypted at rest — `infra/crypto/`), the **try-it** family (`contracts/tryit/`: the catalog of what a version offers, an HTTP request to an OpenAPI operation, a Kafka publish (writers only) or tail read, a read-only SQL sample of an ODCS dataset — each measured against the document as `CONFORMANCE` findings, never stored) and the hierarchy tree) with its SPA (the Hierarchy tree at `/`, the Contracts list and pages with the history timeline and the Follow button, the CodeMirror editor with live findings and Save-anyway, the Try it drawer, import, diff, download, export, the Environments page, and the **contract reader** — the version page's Reader view over the server's render model (`GET …/{vid}/model`, `contracts/render/`) with a Source toggle, a table of contents and findings that deep-link into the rendered element), the **Errors report** — the catalog-wide findings view over the stored check snapshots at `/errors` (the paged `GET /contracts/errors` + `GET /contracts/errors/facets` pair), the **compatibility report** between two arbitrary versions of a contract (`GET …/{vid}/compatibility`, `contracts/checks/Compatibility.kt` — both directions' breaking-change facts, the two-way verdict and the SemVer bump, never stored), the **inference engine** (`contracts/infer/`: `POST /contracts/infer` derives a DRAFT document from samples — HTTP exchanges → OpenAPI 3.1, message payloads → AsyncAPI 3.0, a relation description or JSON rows → ODCS 3.0.2 — through one JSON → JSON Schema inferencer, every heuristic reported as an `INFERENCE` note; the three observe legs pull ONE sample through an Environment with the try-it legs' own code; the SPA's Infer page at `/contracts/infer` and `/contracts/:id/infer` hands the draft to the editor — nothing is stored until the ordinary create path saves it), and every quality gate. Every feature landed in the shape of the feature template below; the next one does too.
