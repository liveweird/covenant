...
**Implemented so far (the scaffold):** the full stack + tooling + auth surface — JWT login with a sliding refresh pair and a revocation blocklist, opt-in email MFA, self-service password reset, per-account lockout and per-IP rate limits, ADMIN-managed accounts and per-user feature flags, the synced per-user UI/email language (EN/PL), the React shell (nav model, command palette, theme, changelog), the checker's HTTP contract, and every quality gate. The contract catalog lands feature by feature (teams → domains/systems → contracts/versions → validators → checker engines → import/diff/export) — each in the shape of the feature template below.
