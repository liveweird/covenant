 ...
 * Every read and mutation runs over `active()`; the list's count and rows share ONE predicate,
...
class NotificationService(private val database: R2dbcDatabase) {
    ...
    suspend fun read(id: UInt): NotificationResponse? = suspendTransaction(database) {
        Notifications.selectAll().where { (Notifications.id eq id) and active() }.map { it.toResponse() }.singleOrNull()
    }

    suspend fun markSeen(id: UInt): Int = suspendTransaction(database) {
        Notifications.update({ (Notifications.id eq id) and active() }) { it[wasSeen] = true }
    }

    suspend fun markUnseen(id: UInt): Int = suspendTransaction(database) {
        Notifications.update({ (Notifications.id eq id) and active() }) { it[wasSeen] = false }
    }
    ...
    suspend fun markAllSeen(recipientId: UInt): Int = suspendTransaction(database) {
        Notifications.update({ (Notifications.recipientId eq recipientId) and (Notifications.wasSeen eq false) and active() }) {
            it[wasSeen] = true
        }
    }

    suspend fun delete(id: UInt): Int = suspendTransaction(database) {
        Notifications.update({ (Notifications.id eq id) and active() }) { it[markedAsDeleted] = true }
    }

    suspend fun list(recipientId: UInt, wasSeen: Boolean?, paging: PageRequest): NotificationListResult = suspendTransaction(database) {
        var predicate: Op<Boolean> = (Notifications.recipientId eq recipientId) and active()
