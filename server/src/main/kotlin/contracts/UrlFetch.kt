...
/**
 * The server side of "import from a URL": fetches a contract document (OpenAPI/AsyncAPI/ODCS) the user points at and
 ...
 * `contract.fetched` scheme/host (the same never-log-the-full-URL discipline as
...
/** The 400 detail for a rejected source reference on a contract-version write. */
 ...
 * full-guard [ContractUrlFetcher]), so the suite can exercise the ROUTE's 200/502 paths against
 * a 127.0.0.1 fixture server. Production never sets it.
 */
val ContractUrlFetcherKey = AttributeKey<ContractUrlFetcher>("ContractUrlFetcher")
 ...
 */
class ContractUrlFetcher(private val urlValidator: (String) -> URI = ::validateFetchUrl) {
