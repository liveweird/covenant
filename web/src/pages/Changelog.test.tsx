...
const STORAGE_KEY = "covenant.changelog";
...
describe("Changelog page", () => {
  ...
  test("shows the English bodies by default", () => {
    ...
    expect(screen.getByText("Administrators manage accounts and per-user feature flags.")).toBeInTheDocument();
  });
  ...
  test("shows the Polish bodies when the UI language is Polish", async () => {
    ...
    expect(screen.getByText("Administratorzy/rki zarządzają kontami i flagami funkcji per użytkownik/czka.")).toBeInTheDocument();
    expect(screen.queryByText("Administrators manage accounts and per-user feature flags.")).not.toBeInTheDocument();
  });
