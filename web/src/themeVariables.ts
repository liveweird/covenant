...
/**
 * The colour tokens Covenant owns outside Mantine's defaults (Lettuce's v3.3.0 pass, ported) — every value here
...
export const LIGHT_TOKENS = {
  text: "#1f2328",
  dimmed: "#5f6b76",
  // A faint violet cast (not Lettuce's neutral #f8f9fb) — dimmed ink stays 5.0:1 on it.
  canvas: "#f6f5fa",
  // The quiet tint under filter panels (FilterPanelBody) — dimmed ink stays ≥ 4.5:1.
  surfaceTint: "#f8f7fb",
  border: "#dee2e6",
  borderStrong: "#ced4da",
  error: "#c92a2a",
  inkWarning: "#b23a0a",
  inkError: "#c92a2a",
  // The brand hue where it must be PERCEIVED against the canvas (nav marker, hover underline,
  // focus) — the light primary here, the 4-shade in the dark scheme (see theme.ts).
  accentInk: "#6a32bd",
} as const;
...
function schemeVariables(tokens: typeof LIGHT_TOKENS | typeof DARK_TOKENS): Record<string, string> {
  return {
    ...
    "--covenant-canvas": tokens.canvas,
    "--covenant-surface-tint": tokens.surfaceTint,
    "--mantine-color-default-border": tokens.border,
    "--covenant-border-strong": tokens.borderStrong,
    "--mantine-color-error": tokens.error,
    "--covenant-ink-warning": tokens.inkWarning,
    "--covenant-ink-error": tokens.inkError,
    "--covenant-accent-ink": tokens.accentInk,
  };
}
