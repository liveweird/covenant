...
import ContractToadieAdoptions from "./ContractToadieAdoptions";
...
export default function ContractToadieUsage({ contractId, canWrite, consumersOnly = false, onReviewReady }: { contractId: number; canWrite: boolean; consumersOnly?: boolean; onReviewReady?: (ready: boolean) => void }) {
  ...
  const [adoptionsFetching, setAdoptionsFetching] = useState(true);
  ...
  useEffect(() => { onReviewReady?.(!links.isFetching && !usage.isFetching && !adoptionsFetching); }, [onReviewReady, links.isFetching, usage.isFetching, adoptionsFetching]);
  useEffect(() => {
  ...
  async function refresh() {
    ...
    try {
      ...
      await refreshQueriesAfterMutation(queryClient, ["contracts"]);
    } catch (error) {
  ...
  return (
    <Stack gap="sm">
      <Stack gap="sm" role="region" aria-label={t("toadie.usage.title")}>
        ...
        {editing && links.data && <ToadieLinksModal contractId={contractId} links={links.data} onClose={() => setEditing(false)} onSaved={async () => { setEditing(false); await refreshQueriesAfterMutation(queryClient, ["contracts"]); }} />}
      </Stack>
      <ContractToadieAdoptions contractId={contractId} onFetchingChange={setAdoptionsFetching} />
    </Stack>
  );
}
